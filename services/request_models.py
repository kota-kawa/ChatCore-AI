...
# 共有トークンは secrets.token_urlsafe(32) 由来なので余裕をもって上限を設ける。
# Share tokens come from secrets.token_urlsafe(32); cap with generous headroom.
...
# English: Request payload for forking a shared chat into the viewer's own room.
class ForkSharedChatRoomRequest(RequestPayloadModel):
    token: Annotated[str, Field(min_length=1, max_length=MAX_SHARE_TOKEN_LENGTH)]
    # 複製先のルームID。既存のルーム作成APIと同じくクライアントが採番する。
    # Target room id, generated by the client exactly like the room creation API.
    id: ChatRoomIdStr
