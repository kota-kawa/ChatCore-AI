...
def build_web_search_system_message(
    ...
    *,
    ...
    lines = [
        ...
        "This applies to requests to see something as well: when the user asks for photos, images, or what something looks like, answer with a concrete description drawn from the sources (scale, shape, material, color, setting, season, distinguishing features), not with links to pages that hold pictures. The application attaches an illustrative image on its own when a suitable one exists, so never promise, announce, or substitute for it.",
