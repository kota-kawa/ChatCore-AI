...
- 使える表現例: カード、タイムライン、マトリクス、マップ風レイアウト、ランキング、ステータスボード、タブ、フィルタ、トグル、スライダー、クリックで展開する詳細、軽いCSS transition。内容に合わない表現は使わないでください。
- 見た目は「ただのHTML表」ではなく、要点が一目で分かる小さなプロダクトUIとして設計してください。モバイルでも読めるレスポンシブ構成、十分なコントラスト、短いラベル、明確な状態表示を優先してください。
...
- `html` には初期表示に必要な骨組みを入れ、CSSは `css`、JavaScriptは `js` に分けてください。HTML内に `<script>` や `<style>` を入れないでください。
