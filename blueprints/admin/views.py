...
# SQLAlchemyのPostgreSQL識別子quoteを使って安全なSQL識別子を構築する。
# Construct a safely quoted SQLAlchemy PostgreSQL identifier.
...
async def _load_dashboard_data(selected_table: Optional[str]) -> dict:
    ...
    """
    return await repository_load_dashboard_data(selected_table)


# データベースにテーブルを新規作成する関数
# Execute CREATE TABLE in the database and commit the transaction.
async def _create_table_in_db(table_name: str, column_definitions: str, table_options: str) -> None:
    """
    ...
    parsed_columns = _parse_column_definitions(column_definitions)
    validated_options = _validate_table_options(table_options)
    await repository_create_table(table_name, parsed_columns, validated_options)


# 指定テーブルが存在する場合にそれを削除する関数
# Drop the table if it exists in the database schema and commit.
async def _drop_table_if_exists(table_name: str) -> bool:
    """
    ...
    """
    return await repository_drop_table_if_exists(table_name)


# テーブル名と重複をチェックした上でカラムを追加する関数
# Check table existence and duplicate column names before executing ALTER TABLE ADD COLUMN.
async def _add_column_if_valid(table_name: str, column_name: str, column_type: str) -> str:
    """
    ...
    """
    parsed_definition = _parse_column_definition(f"{column_name} {column_type}")
    return await repository_add_column_if_valid(
        table_name,
        str(parsed_definition["name"]),
        str(parsed_definition["type"]),
        [str(item) for item in parsed_definition["modifiers"]],
...
async def _drop_column_if_valid(table_name: str, column_name: str) -> tuple[str, Optional[str]]:
    ...
    """
    return await repository_drop_column_if_valid(table_name, column_name)
