...
Redis 障害時の扱いは用途ごとに異なります。一般キャッシュの未ヒットやシングルフライトのフェイルオープンは可能ですが、セッション本体とGoogle OAuthトランザクションは安全側へ倒します。OAuth開始時にトランザクションを保存できなければ認証プロバイダーへ遷移せず、セッション保存に失敗した場合は機密値をCookieへ書き込まず必要に応じて再認証させます。通常のセッション更新は読み込み時の内容との楽観的排他を行い、古いリクエストの全体スナップショット書き戻しを拒否します。ログイン ID は認証成功時にローテーションされます。
