...
ユーザーが発話へ直接貼り付けた URL は別経路で最大3件を並列取得し、1件の取得結果を待つ時間を最大10秒、1ターンの合計待機を最大12秒に制限します。現在は並列開始のため10秒が実効上限です。待機を打ち切った取得処理も含め、URL取得の同時実行は全経路で最大12件です。各 DNS 回答の IPv4／IPv6 候補がすべて global IP かつ非 multicast であることと HTTP:80／HTTPS:443 を確認してから接続先 IP を固定し、リダイレクト先も同じ検査を通します。取得本文は直近の user message に信頼できない参照資料としてエスケープして付与します。
