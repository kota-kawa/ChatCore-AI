...
LLM のストリーム終了理由が出力上限の場合は成功完了として扱いません。回答を書いた判断ステップが出力上限で切れたときは、`services/chat_answer_continuation.py` が表示済みの生本文を assistant 履歴として渡す限定回数の継続生成を行い、境界重複を除去します。継続は別フェーズではなく同じ回答の続きであり、モデルへ渡すのは判断ループが使ったのと同じ要求です。継続が最初から書き直した場合は既存本文の末尾を錨に接合し、接合できないパスは破棄します。正常終了しても新しい本文が増えない継続は `incomplete` として扱います。検索トレースなどの表示装飾と内部の状態更新封筒は継続入力にも表示本文にも含めません。継続上限後も本文がある場合は一度だけ保存し、終端イベント `incomplete` で部分回答であることを明示します。ツール呼び出しの有無が確定するまで判断ステップは試行全体をバッファするため、一時障害時に途中出力を破棄して先頭から安全に再試行できます。再試行が尽きて失敗した場合も、このバッファに本文が残っていれば捨てずに配信・保存し、`incomplete` で締めます。`error` で終わるのは本文を1文字も生成できなかったターンだけです。調査ステップがプロバイダ障害で落ちた場合は、ツールを外した要求へ1度だけ縮退し、収集済みの根拠だけで回答します。仕上げ（正規化・引用解決・保存）まで含めて例外を捕捉し、どの経路でも終端イベントを必ず1つ発行します。プロバイダが自前の見積もりより厳しく入力超過を返した場合は、同じ要求を送り直さず、生のツール結果を落とし、完全な `TurnState`・最新入力・直前の会話を残した最小要求で同じ判断を1度だけやり直します。判断ループの最後のターンが本文を返さなかった場合（内部封筒だけ、無出力、出力上限で本文ゼロ）は、回答のみの要求で同じ判断を1度だけやり直し、それでも本文が無ければ `error` で終えます。検索画像だけ、またはステップのトレースだけの応答は回答として保存しません。
...
`services/chat_agent_budget.py` は `get_evidence` と `read_web_page` の読み取り予算を合わせて管理します。既定値は1ターン12回・応答合計48,000文字・1応答4,000文字で、検索の実行回数・根拠予算とは別です。`WebPageReader` の外部取得は失敗も含め最大3ページ、取得結果を待つ時間の合計は30秒、1ページの待機は最大10秒です。合計待機を打ち切った後は新規取得せず、既存の取得処理は自身のネットワークタイムアウトまで残る場合があります。本文抽出には既存の30,000文字・受信約300,000バイトの上限があるため、`next_start` がなくても元ページの全内容を取得できたとは限りません。

ユーザーが発話へ直接貼り付けた URL は別経路で最大3件を並列取得し、1件の取得結果を待つ時間を最大10秒、1ターンの合計待機を最大12秒に制限します。現在は並列開始のため10秒が実効上限です。待機を打ち切った取得処理も含め、URL取得の同時実行は全経路で最大12件です。各 DNS 回答の IPv4／IPv6 候補がすべて global IP かつ非 multicast であることと HTTP:80／HTTPS:443 を確認してから接続先 IP を固定し、リダイレクト先も同じ検査を通します。取得本文は `services/chat_url_context.py` が抜粋と全文へ分け、抜粋だけを直近の user message へ信頼できない参照資料としてエスケープして付与します。前置する抜粋は全URL合計で3,000トークン（1件あたり最低700トークン）に収め、本文の純粋な先頭一致として切り出すため、続きの読み出し開始位置が抜粋の文字数と一致します。全文（抽出上限の30,000文字まで）は `EvidenceStore.add_pasted_page` で Web 根拠として登録し、`WebPageReader.seed_page` で当該ターンのページキャッシュへ種として渡します。これによりモデルは `read_web_page(evidence_id, start, length)` で残りを再取得なしに分割して読め、`TurnState` には `tool_name="pasted_url"` の実行として記録されます。以前は取得本文を全量そのまま前置していたため、長いページでは直近履歴のトークン予算を超えた分が黙って切り落とされ、モデルが途中までしか読めない状態になっていました。再生成（`chat_regenerate` / `chat_edit_and_regenerate`）でも同じ取得・前置を行います。取得は `fetch_pasted_url_context` に集約し、送信経路と再生成経路が同じ条件（最大3件・抜粋予算・抽出上限）でリンク先を読みます。再生成時に取得しないと、モデルはリンク先へ到達する手段を持たないまま URL 文字列だけを見て答えることになります。過去ターンで貼られた URL（直近10発話・最大5件）は、本文を取得せずに根拠として登録するだけに留めます。`read_web_page` が読むときにはじめて既存の SSRF 対策付き経路で取得するため、参照されなかった URL は通信を起こしません。収集元は参照ブロックを前置する前の生のユーザー発話に限定します。前置後の文面から抽出すると、外部ページ本文や添付本文に含まれるリンクまで「ユーザーが貼った URL」として取得対象になってしまうためです。
