...
- 白から薄い灰緑の面に、濃いグレーの文字、薄い黒の罫線、緑 1 色のアクセント。アクセント以外の色相は意味色（成功・失敗・情報）だけに限ります。
- ダークテーマは `:root[data-theme="dark"]` で上書きされ、紺系の面（`#0f172a`／`#111827`）に同じ緑を載せます。切替は `scripts/core/theme.ts` が `data-theme` 属性で行います。
...
2. 現在のアクセントは緑 1 色です。揃えるなら、白地に白文字の面は `--primary-dark`、緑面の文字は `--text-dark` か `--modal-accent-text`（ダークの緑面用 `#062c1e`、6.6:1）が既存の組み合わせです。アクセントを変えたい場合は `--primary-*` を差し替えて全画面に反映させます。
...
4. 色は `variables.css` のトークン経由にすると、後から全体を変えやすくなります。既存ファイルには生の 16 進数が多数残っています（例: `memo/static/css/memo_form.css`）が、触った箇所だけ近いトークンへ置き換えれば十分で、範囲を超える一括置換は不要です。
