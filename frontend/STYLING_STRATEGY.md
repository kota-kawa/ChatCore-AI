...
- **`pages/user_settings/user_settings.css`** (4,161 lines) — fails both. It defines the `edit-prompt-modal__*` and `prompt-card__*` families, which `/prompt_share` and `/prompt_share/manage_prompts` render too, and 55 of its names overlap the rest of the bundle.
