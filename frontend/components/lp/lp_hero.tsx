      ...
      <div className="lp-demo__window">
        ...
        <div className="lp-demo__body">
          <div className="lp-demo__msg lp-demo__msg--user">
            {locale === "en" ? "Compare three competitors’ pricing pages and summarize the key points." : "競合3社の料金ページを比較して、要点を3行でまとめて"}
          </div>
          <div className="lp-demo__msg lp-demo__msg--ai">
            <p>{locale === "en" ? "Here are the three key points." : "比較の要点は次の3つです。"}</p>
            <ul>
              <li>{locale === "en" ? "Company A charges a flat monthly fee; B and C use metered pricing." : "A社は月額固定、B社・C社は従量課金が基本"}</li>
              <li>{locale === "en" ? "Only Company B offers a free tier." : "無料枠があるのはB社のみ（月1,000回まで）"}</li>
              <li>{locale === "en" ? "Company C has the largest annual discount at 20%." : "年契約の割引率はC社が最も大きい（20%）"}</li>
            </ul>
          </div>
          <div className="lp-demo__actions">
            <span className="lp-demo__chip lp-demo__chip--memo">
              <i className="bi bi-journal-check"></i> {locale === "en" ? "Saved as a memo" : "メモに保存しました"}
            </span>
            <span className="lp-demo__chip lp-demo__chip--share">
              <i className="bi bi-people"></i> {locale === "en" ? "Share prompt" : "プロンプトを共有"}
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <section className="lp-hero">
      <span className="lp-hero__kicker" aria-hidden="true">
        {locale === "en" ? "Make every conversation useful." : "対話を、資産に。"}
      </span>
      <div className="lp-container lp-hero__inner">
        <div className="lp-hero__copy">
          <p className="lp-eyebrow">{locale === "en" ? "AI CHAT × PROMPT LIBRARY × MEMOS" : "AIチャット × プロンプト共有 × メモ管理"}</p>
          <h1 className="lp-hero__title">
            {locale === "en" ? "Turn AI conversations" : "AIとの対話を、"}
            <br />
            {locale === "en" ? "into lasting results." : "成果として残す。"}
          </h1>
          ...
          <ul className="lp-hero__trust">
            ...
            <li>{locale === "en" ? "No credit card" : "クレジットカード不要"}</li>
            <li>{locale === "en" ? "Works in your browser" : "ブラウザだけで使える"}</li>
          </ul>
        </div>
        <LpHeroDemo />
      </div>
