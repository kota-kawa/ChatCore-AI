...
export function SkillDetailModal({ skill, onClose }: SkillDetailModalProps) {
  ...
  return (
    <div
      ref={modalRef}
      id="skillDetailModal"
      className={`skill-detail-modal modal-base ${isOpen ? "is-open" : ""}`.trim()}
      role="dialog"
      aria-modal="true"
    ...
    >
      {skill ? (
        <div className="skill-detail-modal__content" role="document">
          ...
          <p className="skill-detail-modal__lead">{t("home.skillDetailsDescription")}</p>

          <section className="skill-detail-modal__section" aria-labelledby="skill-detail-instructions-title">
            <div className="skill-detail-modal__section-heading">
              <h3 id="skill-detail-instructions-title">{t("home.skillInstructions")}</h3>
              <span>{t("home.skillCharacterCount", { count: skill.instructions.length.toLocaleString() })}</span>
            </div>
            <pre className="skill-detail-modal__instructions">{skill.instructions}</pre>
          </section>
        </div>
      ) : null}
    </div>
