...
- Web 検索に関する比較では、変更前後に同じ質問と固定した検索結果を渡します。検索の要否、クエリ、採用した情報、要約、引用を変更箇所に応じて比較し、情報の選択が適切か、出典が主張を支えているか、検索結果にない事実を足していないかを判定します。入力・使用した検索結果・観点・変更前後の結果を PR 本文に記録してください。モデルは上の役割表にある LLM 代替サブエージェントを使います。
...
- 出力の劣化は文章を読むだけでは判定できないため、変更前後で同じ入力を流し、`services/chat_generation_telemetry.py` が記録する指標を並べます。指標は構造化ログ（既定で `logs/app.log`）の JSON 行に `request_id` とともに出力されるので、該当ターンの行から次のキーを取り出して PR 本文に書きます。
  ...
  - 失敗からの縮退: `empty_answer_recoveries`、`tool_schema_retries`、`tool_schema_recoveries`、`research_failure_recoveries`、`salvaged_partial_answers`（縮退の原因は `tool_schema_rejections` と `workspace_tool_results` で確認します）
