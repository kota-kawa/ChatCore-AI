...
Claudeで自動接続（DCR）が失敗する場合は、Chat-Coreへログイン後、設定画面の「セキュリティ」から
「Claude接続用認証情報」を発行します。Claudeのカスタムコネクタで正規の接続先URLを指定し、詳細設定に表示された
クライアントIDとクライアントシークレットを入力してください。認証情報はユーザー専用で、シークレットは発行時だけ表示されます。
再発行すると以前の認証情報と、その認証情報で作成した連携は失効します。
