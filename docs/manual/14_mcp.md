...
チャットから自動抽出された内容は、ユーザーが「マイコンテキスト」で承認するまでは保存候補として
分離されます。未承認・却下済みの候補がMCPツールの検索結果やダイジェストへ返ることはありません。

マイコンテキストのJSON／Markdown書き出しと、プレビュー確認を伴う読み込みはWeb画面専用です。
MCPツールからファイルの書き出しや一括読み込みを開始することはできません。
