...
erDiagram
    ...
    users ||--o{ chat_tool_approvals : proposes
    chat_history ||--o{ chat_tool_approvals : carries
    users ||--o{ chat_tool_auto_approvals : grants
    ...
    prompts ||--o| prompt_impression_counts : shows
...
| Chat | `chat_rooms`, `chat_history`, `shared_chat_rooms`, `chat_room_summaries`, `memory_facts`, `user_skills`, `chat_tool_approvals`, `chat_tool_auto_approvals` | 永続化境界は機能別に分かれ、ルーム・履歴・分岐・共有・ルーム内メモリを `services/repositories/chat_repository.py` が、`user_skills` と既定スキルの ON/OFF（`users.generative_ui_skill_enabled`、`users.memo_tools_skill_enabled`）を `services/repositories/user_skill_repository.py` が、承認カードと「常に承認」の付与を `services/repositories/chat_tool_approval_repository.py` が所有する。部屋削除は履歴・要約・ルーム内メモリ・承認カードへ cascade する。`chat_rooms.mode` は `normal`／`temporary`。`chat_rooms.last_activity_at` はメッセージ保存時に更新され、履歴一覧の新着順とカーソル境界の正本になる。`chat_rooms.pinned_at` はサイドバーのピン留めで、NULL がピン留めなしを表す。ピン留めした部屋はページング対象の履歴一覧から外れ、最初のページに `pinned_rooms` として全件（件数上限なし）をピン留めした新しい順で返す。付け外しでは `last_activity_at` を更新しない。`shared_chat_rooms.revoked_at`／`expires_at` は共有リンクのライフサイクルで、どちらも NULL の行だけが公開読み取りの対象になる。`user_skills` はユーザーがトップページで登録する再利用指示で、有効行だけがチャット生成時の追加システム文脈になる。生成UIとメモは、コード定義の削除・編集不可なデフォルトSkillであり、ユーザーごとのON/OFFだけを `users` の列に保存するため、`user_skills` の件数上限には含めない。共有 Skill から追加した行は `source_prompt_id` で出典と追加済み状態を追跡し、共有元が削除された場合も Skill 本体は残して参照だけを NULL にする。`chat_tool_approvals` は書き込みツール（メモの作成・追記・書き換え）の承認カード1件を1行に持ち、`assistant_message_id` は回答の保存まで NULL、`status` は `pending`／`succeeded`／`failed`／`denied`／`expired`／`superseded`／`cancelled`。`chat_tool_auto_approvals` は利用者とツール名の組で一意な「常に承認」の付与で、取り消しは行の削除。両テーブルの値の集合は `services/response_models.py` の Literal を正本にし、DB の CHECK 制約はそこから組み立てる（[ADR 0013](../decisions/0013-chat-writes-through-stored-approvals.md)）。 |
