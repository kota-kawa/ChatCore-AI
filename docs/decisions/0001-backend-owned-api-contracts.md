...
したがって、バックエンドの Pydantic model を変更したら `npm run generate:api-schemas` を実行するという `docs/knowledge/development_conventions.md` の要求は、これまでよりも重要になります。再生成を忘れると、上表のフィールドはチャット画面で黙って `undefined` / 既定値に落ち、フォールバックのテスト（`frontend/tests/api_contract.test.ts`）でも検知できません。
