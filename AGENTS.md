...
- AI の出力が変わる変更（プロンプト文言、ツール定義、会話やコンテキストの組み立て、モデル設定、判断ループ）は、PR を作る前に LLM 代替サブエージェントに変更前と変更後のプロンプトを同じ入力で与え、出力を並べて比較し、品質が上がったことを確認してください。この比較は本番モデルの挙動を再現しないため、確認できる範囲と限界、手順は `docs/knowledge/development_conventions.md` の「AI 出力の品質確認」に従います。比較に使った入力・観点・結果を PR 本文に書きます。
