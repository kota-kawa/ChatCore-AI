...
MODEL_REGISTRY: list[tuple[str, type[BaseModel]]] = [
    ...
    ("PinChatRoomRequest", PinChatRoomRequest),
    ("ShareChatRoomRequest", ShareChatRoomRequest),
    ("ForkSharedChatRoomRequest", ForkSharedChatRoomRequest),
    ("ChatMessageRequest", ChatMessageRequest),
    ("ToolApprovalDecisionRequest", ToolApprovalDecisionRequest),
    ...
    ("PromptImpressionRequest", PromptImpressionRequest),
    ...
    ("UsageLimitWindowResponse", UsageLimitWindowResponse),
    ("UsageLimitsResponse", UsageLimitsResponse),
    ...
    ("ToolApprovalApi", ToolApprovalApi),
    ("ToolApprovalDecisionResponse", ToolApprovalDecisionResponse),
    ("ToolApprovalConflictResponse", ToolApprovalConflictResponse),
    ("ToolAutoApprovalApi", ToolAutoApprovalApi),
    ("ToolAutoApprovalsResponse", ToolAutoApprovalsResponse),
    ("ToolAutoApprovalRevokeResponse", ToolAutoApprovalRevokeResponse),
...
def _collect_model_schemas() -> list[tuple[str, dict]]:
    ...
    # English: For each Pydantic model in the model registry, generate its JSON schema,
    #          resolve recursive definitions, and append to the list.
...
def get_schema_fingerprint() -> str:
    model_schemas = dict(_collect_model_schemas())
    payload = json.dumps(
        model_schemas,
        ensure_ascii=False,
        sort_keys=True,
...
def _schema_without_defs(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    # English: Recursively traverse the schema tree, replacing any encountered `$ref` reference definitions
    #          with their actual object contents.
    ...
    dereferenced = dereference(schema)
    # 日本語: 再帰関数は任意ノードを返すため、トップレベルが辞書であることを明示します。
    # English: The recursive helper returns arbitrary nodes, so assert the top level is a mapping.
    return dereferenced if isinstance(dereferenced, dict) else {}


# 日本語: json-schema-to-zod ツールが生成した JS/TS コードの定数宣言を、
#         フロントエンドで利用可能な `export const ...Schema = ...` 形式に正規化します。
# English: Normalize the constant declarations in the JS/TS code generated by json-schema-to-zod
#          into `export const ...Schema = ...` format for frontend usage.
...
# English: Receive a JSON schema and convert it into Zod schema TypeScript code
#          by executing the `json-schema-to-zod` npm package as a subprocess.
...
# English: Main entry point for the schema generation process.
#          Handles collection, conversion, type inference addition, and writes to `api_schemas.ts`.
def main() -> None:
    ...
    # English: Convert each collected model schema into a Zod schema definition,
    #          and append it along with TypeScript type inference definitions (z.infer) to the output list.
